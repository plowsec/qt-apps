...
bool MainWindow::isValid(int grilleTest[9][9],int position)
{
    ...
    {
        if(box->isChecked())
            report+="Case "+QString::number(position)+" already done :o\n";
    ...
    for(int n=1;n<=9;n++)
    {
        if(Free(grilleTest,n,i,j))
        {
            grilleTest[i][j]=n;
            if(box->isChecked())
                report+="Trying "+QString::number(n)+" for the field  "+QString::number(position)+"...\n";
...
//            addText(grilleTest[m][n]+"");
    ...
    addText("Number of times the algorithm was executed : "+QString::number(number)+"\n"+report);
